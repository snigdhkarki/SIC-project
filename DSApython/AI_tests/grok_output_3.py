# Input sentences (includes modifiers for robustness)
...
def parse_sentence(sentence):
    ...
    for i, word in enumerate(words):
        if word.endswith("s"):
            ...
            break
    if V_index == -1:
        raise ValueError(f"No verb found in sentence: {sentence}")
    ...
    for i in range(V_index - 1, -1, -1):
        if words[i].istitle():
            ...
            break
    if S_index == -1:
        raise ValueError(f"No subject found in sentence: {sentence}")
    ...
    # Extract modifiers
    adj_S = words[:S_index]  # Words before subject (adjectives for subject)
    adv_V = words[S_index + 1:V_index]  # Words between subject and verb (adverbs)
    adj_O = words[V_index + 1:O_index] if O else []  # Words between verb and object (adjectives for object)
...
def process_question(question, list_of_SV_sentences, parsed_sentences, dict_of_relation):
    ...
    if question_word_list[0] in ["Who", "What"] and len(question_word_list) >= 3 and question_word_list[2] not in ["does", "is"]:
        # "Who V O?" or "What V O?" (e.g., "Who eats banana?") - V is inflected
        ...
        # "What does S do to O?" (e.g., "What does Hari do to Gita?") - V is base form
        ...
        # "What does S do?" (e.g., "What does Hari do?") - V is base form
        S = question_word_list[2]
        if S in dict_of_relation:
            for index in dict_of_relation[S]:
                parsed = parsed_sentences[index]
                if parsed['subject'] == S:
        ...
        # "Who/What is Ven/Ved?" (e.g., "Who is eaten?") - V is base form
    ...
    elif question_word_list[0] == "Explain" and question_word_list[1] == "all" and question_word_list[2] == "occurring":
        # "Explain all occurring V." (e.g., "Explain all occurring eats.") - V is inflected
        V = question_word_list[3]
        for index in dict_of_relation.get(V, []):
            print(list_of_SV_sentences[index])

    # Modifier-related questions
    elif question_word_list[0] == "What" and question_word_list[1] == "kind" and question_word_list[2] == "of" and question_word_list[4] != "does":
        # "What kind of S V O?" (e.g., "What kind of Ram eats banana?") - V is inflected
        ...
        # "How does S V O?" (e.g., "How does Ram smashes banana?") - V is inflected
        ...
        # "What kind of O does S V?" (e.g., "What kind of banana does Ram eat?") - V is base form
        O, S, V = question_word_list[3], question_word_list[5], question_word_list[6]
        for index in set(dict_of_relation.get(S, [])) & set(dict_of_relation.get(V, [])) & set(dict_of_relation.get(O, [])):
            parsed = parsed_sentences[index]
            if parsed['subject'] == S and parsed['base_verb'] == V and parsed['object'] == O:
                print(" ".join(parsed['adj_object']) or "none")

# List of predefined questions for testing
questions = [
    "Who eats banana?",
    "What does Hari do?",
...
# Process and test each question
for q in questions:
    print(f"Question: {q}")
